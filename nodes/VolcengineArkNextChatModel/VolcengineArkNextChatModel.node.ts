 ...
 * EngineResponse after tool execution), creating a brand-new `ChatVolcengineForN8n` instance
...
/** Volcengine Ark chat/completions rejects tools whose parameter schema root is missing type:"object". */
function normalizeVolcengineCompletionTool(tool: unknown): unknown {
...
/**
 * Volcengine Ark thinking mode requires prior-turn reasoning_content to be echoed on
 * continuation requests (tools / multi-turn).
 */
function injectVolcengineAssistantReasoningIntoApiMessages(
...
/**
 * ChatOpenAI extension for Volcengine Ark (豆包 / Doubao):
 * - Normalizes tool schemas
 * - Injects reasoning_content across multi-turn tool calls (thinking models)
 * - Mirrors reasoning stream deltas into content for n8n Agent aggregation
 */
class ChatVolcengineForN8n extends ChatOpenAICompletions {
	n8nMirrorReasoningStreamForAgent = false;

	private reasoningContentByToolSignature = new Map<string, string>();
	private volcengineLcMessagesStack: BaseMessage[][] = [];
	...
	protected override _convertChatOpenAIToolToCompletionsTool(
		...
		return normalizeVolcengineCompletionTool(converted) as ReturnType<
	...
	override async _generate(
		...
		this.volcengineLcMessagesStack.push(messages);
		try {
			return await super._generate(messages, options, runManager);
		} finally {
			this.volcengineLcMessagesStack.pop();
		}
	}
...
export class VolcengineArkNextChatModel implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Volcengine Ark Chat Model (Next)',
		name: 'volcengineArkNextChatModel',
		icon: 'file:volcengine.svg',
		group: ['transform'],
		version: [1],
		description:
			'Volcengine Ark (火山方舟) Doubao chat model via OpenAI-compatible API (thinking mode, tool calling). Community node.',
		defaults: {
			name: 'Volcengine Ark Chat Model (Next)',
		},
		codex: {
			categories: ['AI'],
			subcategories: {
		...
		properties: [
			...
			{
				displayName:
					'With thinking mode enabled, sampling parameters such as temperature and top P have no effect on the thinking phase. Use a thinking-capable model such as doubao-seed-1-6-thinking.',
			...
			{
				displayName:
					'When Thinking Mode is Disabled, this node mirrors Doubao `reasoning_content` stream deltas into normal assistant text so n8n AI Agent streaming can aggregate output.',
				...
				options: [
					{
						...
						type: 'options',
						description:
							'Enable Doubao thinking mode to get reasoning_content in responses. Disabled by default for predictable cost and latency. Requires a thinking-capable model (e.g. doubao-seed-1-6-thinking).',
						default: 'disabled',
						options: [
							{
								name: 'Disabled',
								value: 'disabled',
								description: 'Standard chat completion without thinking (this node default).',
							},
							{
								name: 'Enabled',
								value: 'enabled',
								description: 'Enable thinking mode (reasoning_content in responses).',
							},
						],
					},
					{
					...
					},
					{
						...
						description: 'Sampling temperature in [0, 2]. Higher values produce more varied output.',
					},
					{
						displayName: 'Top P',
						name: 'topP',
					...
					},
					{
						...
						default: '',
						description:
							'Optional extra fields merged (shallow) into modelKwargs. Use to pass Ark-specific parameters not exposed above (e.g. {"logprobs": true}).',
					},
				],
			},
		],
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		if (thinkingMode === 'enabled') {
			modelKwargs.thinking = { type: 'enabled' as const };
		}
		...
		const model = new ChatVolcengineForN8n({
			model: modelName,
			apiKey,
